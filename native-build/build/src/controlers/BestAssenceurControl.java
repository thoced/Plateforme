...
public class BestAssenceurControl extends AbstractControl implements Savable,Cloneable {
    ...
    protected void controlUpdate(float tpf) {
            ...
            if(spacing > 0.1f) {
                ...
                Vector3f velocity = direction.normalize();
                this.getSpatial().setLocalTranslation(this.getSpatial().getLocalTranslation().add(velocity.mult(tpf * speed)));
                direction = destination.getLocalTranslation().subtract(Vector3f.ZERO);
                spacing -= velocity.normalize().mult(tpf * speed).length();
            }
